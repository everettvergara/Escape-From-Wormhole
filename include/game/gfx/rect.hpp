...
namespace g80::game::gfx {
    ...
    public:
        ...
        auto draw(const point &p, const int_type w, const int_type h, const Uint32 rgba) -> void;
        auto draw(const point &p, const int_type w, const int_type h, const Uint32 rgba, const Uint32 mask) -> void;
        auto draw(const point &p, const int_type w, const int_type h, const palette_gradient &pal, const int ix_from, const int ix_to) -> void; 
        auto draw_s(point p, int_type w, int_type h, const Uint32 rgba) -> void;
        auto draw_s(point p, int_type w, int_type h, const Uint32 rgba, const Uint32 mask) -> void;
        auto draw_s(point p, int_type w, int_type h, const palette_gradient &pal, const int ix_from, const int ix_to) -> void;
    };


    rect::rect(surface *s) : s_(s) {
    }

    auto rect::draw(const point &p, const int_type w, const int_type h, const Uint32 rgba) -> void {
    ...
    auto rect::draw(const point &p, const int_type w, const int_type h, const Uint32 rgba, const Uint32 mask) -> void {
        auto *pixel_top     = (static_cast<Uint32 *>(s_->get_handle()->pixels) + p.x) + (p.y * s_->get_handle()->w);
        auto *pixel_bottom  = h > 0 ? pixel_top + ((h - 1) * s_->get_handle()->w) :
                                    pixel_top + ((h + 1) * s_->get_handle()->w);
        auto *pixel_left    = w > 0 ? pixel_top + s_->get_handle()->w : pixel_top - s_->get_handle()->w;
    ...
    auto rect::draw(const point &p, const int_type w, const int_type h, const palette_gradient &pal, const int ix_from, const int ix_to) -> void {
        auto *pixel_top     = (static_cast<Uint32 *>(s_->get_handle()->pixels) + p.x) + (p.y * s_->get_handle()->w);
        auto *pixel_bottom  = h > 0 ? pixel_top + ((h - 1) * s_->get_handle()->w) :
                                    pixel_top + ((h + 1) * s_->get_handle()->w);
        auto *pixel_left    = w > 0 ? pixel_top + s_->get_handle()->w : pixel_top - s_->get_handle()->w;
    ...
    auto rect::draw_s(point p, int_type w, int_type h, const Uint32 rgba) -> void {
    ...
    auto rect::draw_s(point p, int_type w, int_type h, const Uint32 rgba, const Uint32 mask) -> void {
        {
            int_type x1 = p.x;
            int_type y1 = p.y;
            int_type x2 = p.x + w + (w >= 0 ? -1 : 1);
    ...
    auto rect::draw_s(point p, int_type w, int_type h, const palette_gradient &pal, const int ix_from, const int ix_to) -> void {
